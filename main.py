...
async def planet_weight_received(update, ctx):
    ...
    kb=InlineKeyboardMarkup([[
        InlineKeyboardButton("◀️ Interactive",callback_data="cat_interact"),
...
async def quiz_finish_h(update, ctx):
    ...
    kb=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Again",callback_data="quiz_start_menu"),InlineKeyboardButton("◀️ Menu",callback_data="back")]])
...
async def daily_poll_h(update, ctx):
    ...
    try:
        await ctx.bot.send_poll(chat_id=q.message.chat_id,question=f"🌌 {question}",options=options,is_anonymous=True,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Interactive",callback_data="cat_interact")]]))
    except:
        text=f"📊 *{question}*\n\n"+"".join(f"• {o}\n" for o in options)
