...
def init_worker():
    """
    Start the asyncio event loop in a daemon thread and set up the bot.
    Called by gunicorn's post_fork hook INSIDE each worker process,
    so the loop is guaranteed to run in the right process.
    """
    global bot_loop
