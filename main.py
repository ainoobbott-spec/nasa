...
"""
import os, logging, random, re, requests, asyncio, threading
...
PLANET_DATE = 1
PLANET_WEIGHT = 2
PLANET_CHOICE, PLANET_DATE, PLANET_WEIGHT = range(3)
logging.basicConfig(format="%(asctime)s | %(levelname)s | %(message)s", level=logging.INFO)
logger = logging.getLogger(__name__)

flask_app = Flask(__name__)
tg_app = None
bot_loop = None  # global event loop for the bot
...
QUIZ_QUESTIONS = [
    ...
    {"q":{"ru":"Световой год — это мера...","en":"A light-year measures...","he":"שנת אור מודדת...","ar":"السنة الضوئية تقيس..."},
     "options":["Времени","Расстояния","Массы","Скорости"],"answer":1,
...
async def space_name_h(update, ctx):
    ...
    star_name = f"{prefix} {name[:4].title()} {code}"
    const     = random.choice(["Orion","Lyra","Cygnus","Perseus","Aquila","Centaurus","Vela"])
    spec      = random.choice(["G2V ☀️","K5V 🟠","M4V 🔴","F8V 🟡","A1V 🔵"])
    dist      = random.randint(10,9999)
    random.seed()
    text = (tx(lang,"name_gen_title") +
            f"👨‍🚀 *Callsign:*\n`{callsign}`\n\n"
            f"⭐ *Your star:*\n`{star_name}`\n"
            f"📡 Constellation: {const}  |  Spectral: {spec}\n"
            f"📍 Distance: {dist} light-years\n\n"
            f"📢 Share at @cosmic41!")
...
async def horoscope_date_received(update, ctx):
    ...
    text = (f"🧠 *Q{q_index+1}/10*\n\n"
            f"{'✅' if is_right else '❌'} {result_line}\n\n"
            f"💡 _{exp}_\n\n"
            f"Score: {ctx.user_data['quiz_score']}/{q_index+1}")
...
async def quiz_next_h(update, ctx):
    ...
    if qi >= 10:
        await quiz_finish_h(update, ctx); return
...
async def quiz_answer_h(update, ctx):
    ...
    text = (f"🧠 *Q{q_index+1}/10*\n\n"
            f"{'✅' if is_right else '❌'} {result_line}\n\n"
            f"💡 _{exp}_\n\nScore: {ctx.user_data['quiz_score']}/{q_index+1}")
    await safe_edit(q, text, reply_markup=quiz_kb(lang, q_index, answered=True))

async def quiz_finish_h(update, ctx):
    q     = update.callback_query; await safe_answer(q); lang = get_lang(ctx)
    score = ctx.user_data.get("quiz_score", 0)
    if   score <= 3: grade = {"ru":"🌑 Новичок — продолжай учиться!","en":"🌑 Beginner — keep learning!"}
    elif score <= 6: grade = {"ru":"🌓 Исследователь — хорошее знание!","en":"🌓 Explorer — solid knowledge!"}
    elif score <= 8: grade = {"ru":"🌕 Астронавт — впечатляет!","en":"🌕 Astronaut — impressive!"}
    else:            grade = {"ru":"🚀 Легенда NASA — ты эксперт!","en":"🚀 NASA Legend — true expert!"}
    g = grade.get(lang, grade["en"])
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("🔄 Again", callback_data="quiz_start_menu"),
...
async def capsule_msg_received(update, ctx):
    ...
    if len(user_msg) < 5 or len(user_msg) > 2000:
        await update.message.reply_text("❌ 5–2000 символов"); return CAPSULE_MSG
...
async def capsule_cancel(update, ctx):
    await update.message.reply_text(tx(ctx.user_data.get("capsule_lang","ru"),"capsule_cancel"))
    return ConversationHandler.END
...
async def daily_poll_h(update, ctx):
    ...
    await del_msg(q)
    try:
        await ctx.bot.send_poll(chat_id=q.message.chat_id,
            question=f"🌌 {question}", options=options, is_anonymous=True,
...
async def lunar_calendar_h(update, ctx):
    ...
    phase_names = {0:"🌑 New Moon",2:"🌓 First Quarter",4:"🌕 Full Moon",6:"🌗 Last Quarter"}
    seen  = set()
    for i in range(30):
...
async def job_lunar_alert(context):
    ...
    is_full   = (idx == 4)
    phase_name= "Full Moon 🌕" if is_full else "New Moon 🌑"
    tip = ("📸 Full Moon: great for landscapes! ISO 100, f/11, 1/250s" if is_full
           else "📸 New Moon: perfect for deep sky! ISO 3200, f/2.8, 20-30s")
...
def webhook():
    ...
    # Run async handler in the dedicated bot event loop (thread-safe)
...
# ── STARTUP ───────────────────────────────────────────────────────────────────
def _run_loop(loop):
    """Run asyncio event loop forever in a background thread."""
