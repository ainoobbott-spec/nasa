...
# ── CONFIG ────────────────────────────────────────────────────────────────────
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "8503684628:AAFQltwb59V8ZmkUPZ2pFkCuWh-C0s7ID04")
...
SPACE_FACTS = {
    "ru":["🌌 Вселенной ~13.8 млрд лет. Земля появилась спустя 9 млрд.","⭐ Звёзд во Вселенной больше, чем песчинок на всех пляжах Земли — ~10²⁴.","🌑 Следы Армстронга на Луне сохранятся миллионы лет — там нет ветра.","☀️ Свет от Солнца летит 8 мин 20 сек. От Альфы Центавра — 4.2 года.","🪐 День на Венере длиннее её года.","🌊 На Энцеладе Сатурна бьют гейзеры воды — возможна жизнь.","⚫ Если сжать Землю до горошины — она станет чёрной дырой.","🚀 Вояджер-1 вышел за пределы Солнечной системы в 2012 году."],
    "en":["🌌 The Universe is ~13.8 billion years old. Earth appeared 9 billion years later.","⭐ There are more stars in the Universe than grains of sand on all Earth's beaches.","🌑 Armstrong's footprints on the Moon will last millions of years — no wind there.","☀️ Sunlight takes 8 min 20 sec to reach Earth. Proxima: 4.2 years.","🪐 A day on Venus is longer than its year.","🌊 Saturn's Enceladus has water geysers — life may exist there.","⚫ If Earth were compressed to a marble size, it would become a black hole.","🚀 Voyager 1 entered interstellar space in 2012."],
    "he":["🌌 היקום בן ~13.8 מיליארד שנה. כדור הארץ הופיע 9 מיליארד שנה אחר כך.","⭐ מספר הכוכבים ביקום גדול ממספר גרגרי החול בכל חופי העולם.","🌑 עקבות ארמסטרונג על הירח ישמרו מיליוני שנים — אין שם רוח.","☀️ האור מהשמש מגיע לכדור הארץ תוך 8 דקות ו-20 שניות.","🪐 יום על נוגה ארוך מהשנה שלה.","🌊 לאנקלדוס של שבתאי יש גייזרים של מים.","⚫ אם כדור הארץ היה מתכווץ לגודל גולה — הוא היה הופך לחור שחור.","🚀 ווֹיאַג'ר 1 נכנס למרחב הבין-כוכבי ב-2012."],
    "ar":["🌌 عمر الكون ~13.8 مليار سنة. ظهرت الأرض بعد 9 مليارات سنة.","⭐ عدد النجوم في الكون أكثر من حبات الرمل في جميع شواطئ الأرض.","🌑 آثار أقدام أرمسترونغ على القمر ستبقى ملايين السنين.","☀️ ضوء الشمس يصل إلى الأرض في 8 دقائق و20 ثانية.","🪐 يوم على كوكب الزهرة أطول من سنته.","🌊 لقمر إنسيلادوس التابع لزحل ينابيع مياه.","⚫ لو ضُغطت الأرض إلى حجم كرة رخام، ستصبح ثقباً أسود.","🚀 دخل فوياجر 1 الفضاء النجمي البيني عام 2012."],
}
...
async def send_nasa_image(q, context, queries, cb_data=""):
    ...
    try:
        ...
        if not items:
            await safe_edit(q, tx(lang, "no_img"), reply_markup=back_kb(lang, context=context)); return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(tx("ru", "choose_lang"), parse_mode="Markdown", reply_markup=lang_keyboard())
...
async def setlang_handler(update, context):
    ...
    await safe_edit(q, tx(new_lang, "lang_set") + "\n\n" + tx(new_lang, "start_msg", name=name),
...
async def _send_apod(q, context, params=None):
    ...
    try:
        ...
        caption = f"🌌 *{title}*\n📅 {d}  |  © {copy_}\n\n{expl}…\n\n[🔗 HD]({hdurl})"
        kb = action_kb(lang, "apod_random", "btn_more_rnd", context) if params else back_kb(lang, context=context)
        await del_msg(q)
        if mtype == "image":
            await context.bot.send_photo(chat_id=q.message.chat_id, photo=url, caption=caption, parse_mode="Markdown", reply_markup=kb)
        else:
            await context.bot.send_message(chat_id=q.message.chat_id, text=caption + f"\n\n[▶️]({url})", parse_mode="Markdown", reply_markup=kb)
    except Exception as e:
        await safe_edit(q, f"{tx(lang,'err')} APOD: `{e}`", reply_markup=back_kb(lang, context=context))

async def apod_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    await safe_edit(q, "⏳ APOD..."); await _send_apod(q, context)
...
async def epic_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    await safe_edit(q, "🌍..."); await send_nasa_image(q, context, EARTH_Q, "epic")

async def mars_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🤖...")
    try:
        photos = []
        ...
        if photos:
            ...
            await context.bot.send_photo(chat_id=q.message.chat_id, photo=p["img_src"],
                caption=caption, parse_mode="Markdown", reply_markup=action_kb(lang,"mars","btn_another",context)); return
    except Exception as e:
    ...
    try:
        ...
        for sol in random.sample(list(range(50,1800)),8):
            try:
                ...
                await del_msg(q)
                await context.bot.send_photo(chat_id=q.message.chat_id, photo=img, caption=caption,
                    parse_mode="Markdown", reply_markup=action_kb(lang,"mars_rovers","btn_other_rv",context)); return
            except: continue
...
async def asteroids_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "☄️...")
    try:
        today = date.today().isoformat()
        data  = nasa("/neo/rest/v1/feed", {"start_date":today,"end_date":today})
        neos  = data["near_earth_objects"].get(today,[])
        if not neos:
            await safe_edit(q, tx(lang,"no_data"), reply_markup=back_kb(lang,"asteroids",context)); return
...
async def iss_handler(update, context):
    ...
    try:
        ...
        text = (f"🛸 *ISS — {ts}*\n\n🌍 `{lat:.4f}°` | 🌏 `{lon:.4f}°`\n⚡ ~27,600 km/h  |  🏔 ~408 km\n\n"
                f"👨‍🚀 Crew ({len(iss_crew)}):\n{crew_str}\n\n[🗺 {tx(lang,'iss_map')}](https://www.google.com/maps?q={lat},{lon})")
...
async def gallery_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    await safe_edit(q, "🖼..."); await send_nasa_image(q, context, GALLERY_Q, "gallery")
...
async def channels_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context)
    await safe_edit(q, CHANNELS_TEXT.get(lang, CHANNELS_TEXT["ru"]), reply_markup=back_kb(lang, context=context))
...
def grb_text(lang):
    return {"ru":"💥 *Гамма-всплески (GRB)*\n\nСамые мощные взрывы во Вселенной.\nДлинные — коллапс звезды. Короткие — слияние нейтронных звёзд.",
            ...
            "ar":"💥 *انفجارات أشعة غاما (GRB)*\n\nأقوى الانفجارات في الكون."}.get(lang,"")
...
def radioastro_text(lang):
    return {"ru":"🔭 *Радиоастрономия*\n\nПульсары, реликтовое излучение, нейтральный водород 21 см, квазары, FRB.\nFAST (Китай) 500 м — крупнейший. Сигнал Wow! (1977) не объяснён.",
            ...
            "he":"🔭 *רדיו אסטרונומיה*\n\nפולסרים, CMB, קווזרים, FRB. FAST (סין) 500מ'.",
            "ar":"🔭 *الفلك الراديوي*\n\nنجوم نابضة، CMB، كوازارات. FAST (الصين) 500م."}.get(lang,"")
...
def dark_energy_text(lang):
    return {"ru":"⚡ *Тёмная энергия*\n\n68% Вселенной. Открыта 1998 — расширение ускоряется. Нобель 2011.\nПрирода неизвестна — главная загадка космологии.",
            ...
            "he":"⚡ *אנרגיה אפלה*\n\n68% מהיקום. התגלתה 1998. נובל 2011.",
            "ar":"⚡ *الطاقة المظلمة*\n\n68% من الكون. نوبل 2011."}.get(lang,"")
...
def ozone_text(lang):
    return {"ru":"🛡 *Озоновый слой*\n\nЗащищает от УФ. Монреальский протокол (1987) — запрет CFC.\nДыра над Антарктидой медленно затягивается.",
            ...
            "he":"🛡 *שכבת האוזון*\n\nמגנה מ-UV. פרוטוקול מונטריאול (1987).",
            "ar":"🛡 *طبقة الأوزون*\n\nتحمي من الأشعة فوق البنفسجية. بروتوكول مونتريال (1987)."}.get(lang,"")
...
def ocean_cur_text(lang):
    return {"ru":"🌊 *Океанские течения*\n\nГольфстрим, Куросио — переносят тепло, влияют на климат.\nNASA JPL измеряет течения со спутников.",
            ...
            "ar":"🌊 *تيارات المحيط*\n\nتيار الخليج، كوروشيو — ينقلان الحرارة."}.get(lang,"")
...
def sp_stations_text(lang):
    return {"ru":"🛸 *Космические станции*\n\n• *МКС* (с 1998) — 420 т, ~408 км. Экипаж с 2000.\n• *Тяньгун (Китай)* — модульная на НОО.\n• *Gateway* (NASA, ~2028) — у Луны, для Artemis.",
            ...
            "he":"🛸 *תחנות חלל*\n\n• *ISS* (מ-1998) — 420 טון. • *Tiangong (סין)*. • *Gateway* (~2028).",
            "ar":"🛸 *محطات الفضاء*\n\n• *ISS* (منذ 1998) — 420 طن. • *Tiangong (الصين)*. • *Gateway* (~2028)."}.get(lang,"")
...
def women_text(lang):
    return {"ru":"👩‍🚀 *Женщины в космосе*\n\n• Терешкова (1963) — первая.\n• Салли Райд (1983) — первая американка.\n• Савицкая (1984) — первая в открытом космосе.\n• Пегги Уитсон — рекорд по времени.",
            ...
            "he":"👩‍🚀 *נשים בחלל*\n\n• טרשקובה (1963) — הראשונה. • סאלי רייד (1983). • סביצקאיה (1984).",
            "ar":"👩‍🚀 *المرأة في الفضاء*\n\n• تيريشكوفا (1963). • سالي رايد (1983). • سافيتسكايا (1984)."}.get(lang,"")
...
def mars_col_text(lang):
    return {"ru":"🔴 *Колонизация Марса*\n\nSpaceX (Starship), NASA, Китай — планы 2030–2040.\nПроблемы: радиация, гравитация, ресурсы.\nPerseverance тестирует производство кислорода.",
            ...
            "he":"🔴 *קולוניזציה של מאדים*\n\nSpaceX, NASA, סין — תוכניות 2030–2040.",
            "ar":"🔴 *استعمار المريخ*\n\nSpaceX، ناسا، الصين — خطط 2030–2040."}.get(lang,"")
...
def debris_text(lang):
    return {"ru":"🛰 *Космический мусор*\n\n~50 000 отслеживаемых объектов. Скорость ~7.5 км/с.\nМКС маневрирует ~3 раза/год.",
            ...
            "ar":"🛰 *حطام الفضاء*\n\n~50,000 جسم مرصود. السرعة ~7.5 كم/ث."}.get(lang,"")
...
def records_text(lang):
    return {"ru":"🏆 *Рекорды космоса*\n\n• Поляков — 437 суток (Мир, 1994–1995)\n• Кононенко — 1000+ суток (2024)\n• Вояджер-1 — >24 млрд км\n• Parker Probe — 700 000 км/ч",
            ...
            "he":"🏆 *שיאי חלל*\n\n• פוליאקוב — 437 ימים. • Voyager-1 — >24 מיליארד ק\"מ.",
            "ar":"🏆 *أرقام قياسية فضائية*\n\n• بوليكوف — 437 يوماً. • Voyager-1 — >24 مليار كم."}.get(lang,"")
...
def red_giants_text(lang):
    return {"ru":"🔴 *Красные гиганты и эволюция звёзд*\n\n• Солнце → красный гигант через ~5 млрд лет.\n• Сброс оболочки → планетарная туманность → белый карлик.\n• Звёзды >8 M☉ — сверхновая → нейтронная звезда или ЧД.",
            ...
            "ar":"🔴 *العمالقة الحمراء وتطور النجوم*\n\n• الشمس → عملاق أحمر بعد ~5 مليار سنة."}.get(lang,"")
...
async def live_solar_wind_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Solar Wind...")
    try:
        data = None
...
async def live_kp_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Kp...")
    try:
...
async def live_flares_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Flares...")
    try:
...
async def live_iss_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 ISS...")
    try:
...
async def live_radiation_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Radiation...")
    try:
...
async def live_aurora_forecast_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Aurora...")
    try:
        r = requests.get("https://services.swpc.noaa.gov/json/planetary_k_index_1m.json", timeout=12)
        r.raise_for_status()
        data    = r.json()
...
async def live_geomagnetic_alert_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Geomagnetic...")
    try:
        end   = date.today().isoformat()
...
async def live_sunspot_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Sunspots...")
    try:
...
async def live_epic_latest_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 EPIC...")
    try:
        ...
        try:
            await context.bot.send_photo(chat_id=q.message.chat_id, photo=url,
                caption=caption, parse_mode="Markdown", reply_markup=back_kb(lang,"live_epic_latest",context))
        except:
            await context.bot.send_message(chat_id=q.message.chat_id, text=caption+f"\n\n[Open]({url})",
...
async def live_satellite_count_handler(update, context):
    q = update.callback_query; await safe_answer(q)
    lang = get_lang(context); await safe_edit(q, "🔴 Counting...")
    try:
        sl     = get_json("https://api.spacexdata.com/v4/starlink", timeout=10)
        total  = len(sl)
        active = sum(1 for s in sl if isinstance(s,dict) and not (s.get("spaceTrack") or {}).get("DECAY_DATE"))
    ...
    text = (f"🔴 *Satellites (SpaceX Starlink)*\n\nTotal: *{total}*  |  Active: *{active}*\n\n"
            f"Global: ~9,000+ in orbit, ~7,500+ active.")
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "back": await back_handler(update, context); return
    ...
    # ── ИСПРАВЛЕНО: правильный ключ "live_solar_wind" (без мусора) ──
    direct = {
        ...
        "live_solar_wind": live_solar_wind_handler,
        "live_kp": live_kp_handler,
        "live_flares": live_flares_handler,
        "live_iss": live_iss_handler,
...
# ── MAIN (правильная структура для Render + Gunicorn) ─────────────────────────
# Бот запускается в отдельном потоке при старте приложения
...
def run_bot():
    logger.info("🚀 NASA Bot polling started...")
    tg_app.run_polling(allowed_updates=Update.ALL_TYPES)

# Запуск бота при старте (работает с Gunicorn)
