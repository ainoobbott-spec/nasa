...
# ── STARTUP: run bot event loop in background thread ─────────────────────────
...
# Create a dedicated event loop for the bot and start it in a thread
bot_loop = asyncio.new_event_loop()
bot_thread = threading.Thread(target=start_bot_loop, args=(bot_loop,), daemon=True)
bot_thread.start()

# Run setup in the bot loop
future = asyncio.run_coroutine_threadsafe(setup_bot(), bot_loop)
future.result(timeout=30)  # Wait for bot setup to complete before Flask starts
