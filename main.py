...
# Создаем объект приложения бота ПРЯМО ТУТ (вне функций)
...
# Эта функция запустит бота в отдельном потоке
def run_bot():
    logger.info("🚀 NASA Bot polling started...")
    app.run_polling(allowed_updates=Update.ALL_TYPES)

# Запускаем поток бота СРАЗУ при импорте файла
threading.Thread(target=run_bot, daemon=True).start()

# Блок main теперь не обязателен для Gunicorn, но оставим для порядка
